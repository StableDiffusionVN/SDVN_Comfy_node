...
const API_ENDPOINTS = {
	list: "/sdvn/gemini_image_presets",
	save: "/sdvn/gemini_image_presets/save",
	delete: "/sdvn/gemini_image_presets/delete",
};
...
function setStatus(dialog, message, isError = false) {
	const statusEl = dialog?.querySelector(".sdvn-gemini-image-preset-status");
	if (!statusEl) return;
...
function buildDialogContent(selectedName, presetValue, isDefaultPreset) {
	...
	const safeContent = escapeHtml(presetValue ?? "");
	...
	return `
		...
		">
			<div style="padding: 0 0 6px; font-weight: 600; font-size: 15px;">Save/Edit Gemini Image Preset</div>
			<div style="padding: 0 0 10px; font-size: 12px; opacity: 0.75;">
				Preset custom sẽ lưu chung vào một file JSON dạng {"Tên preset": "câu lệnh"}
			</div>
			${readonlyNote}
			<div style="padding: 0 0 10px;">
				<div style="font-size: 12px; opacity: 0.8; padding-bottom: 6px;">Preset name</div>
				<input class="sdvn-gemini-image-preset-name" value="${safeName}" style="
					width: 100%;
					border-radius: 6px;
			...
			<div style="padding: 0;">
				...
				<textarea class="sdvn-gemini-image-preset-content" spellcheck="false" style="
					width: 100%;
					min-height: 280px;
					border-radius: 6px;
					border: 1px solid rgba(255,255,255,0.15);
			...
			<div class="sdvn-gemini-image-preset-status" style="padding: 10px 0 0; min-height: 18px; font-size: 12px; opacity: 0.8;"></div>
			<div style="display: flex; justify-content: space-between; gap: 8px; padding: 14px 0 0;">
				<button class="comfy-btn comfy-btn-secondary sdvn-gemini-image-preset-delete"${isDefaultPreset ? " disabled" : ""}>Delete</button>
				<div style="display: flex; gap: 8px;">
					<button class="comfy-btn comfy-btn-secondary sdvn-gemini-image-preset-cancel">Cancel</button>
					<button class="comfy-btn comfy-btn-primary sdvn-gemini-image-preset-save">Save</button>
				</div>
			</div>
		</div>
	`;
...
async function openPresetDialog(node, state) {
	...
	const nameInput = dialog.querySelector(".sdvn-gemini-image-preset-name");
	const contentTextarea = dialog.querySelector(".sdvn-gemini-image-preset-content");
	const closeDialog = () => {
		dialog?.close?.();
	...
	dialog.querySelector(".sdvn-gemini-image-preset-save")?.addEventListener("click", (event) => {
		event?.preventDefault?.();
		savePreset();
	});
	dialog.querySelector(".sdvn-gemini-image-preset-delete")?.addEventListener("click", (event) => {
		event?.preventDefault?.();
		deletePreset();
	});
	dialog.querySelector(".sdvn-gemini-image-preset-cancel")?.addEventListener("click", (event) => {
		event?.preventDefault?.();
		closeDialog();
	});
...
function ensureButton(node, state) {
	...
	if (!button) return;
	button.tooltip = "Mở popup quản lý preset ảnh Gemini";
...
async function setupNode(node) {
	...
	const presetWidget = node.widgets.find((w) => w.name === PRESET_WIDGET_NAME);
	if (!presetWidget) return;
	const state = (node.__sdvnGeminiImagePresetState ||= {});
